...
for name,x,angle in [('FRONT',-1.4,0),('PROFILE',0,math.pi/2),('BACK',1.4,math.pi)]:
    ...
    for original in bpy.data.collections['HOUND_v01_MODEL_ONLY'].objects:
        obj = original.copy()
        obj.name = 'ReviewOnly_'+name+'_'+original.name
        collection.objects.link(obj)
        obj.matrix_world = transform @ original.matrix_world

ink = bpy.data.materials.new('Hound01_Label')
...
label('BoardVersion','VOLUMEN 01  /  1,80 m',2.04,2.05,.05,'RIGHT')
...
board.camera = camera
board.render.filepath = 'D:/Projects/Gloom/docs/art/hound/blockout-v01/turnaround.png'
